      ...
      <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1.5" data-testid="password-strength-meter">
        {rules.map((r) => (
          <span
            ...
            style={{ color: r.test ? '#BFDBFE' : '#BFDBFE' }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full transition-colors"
              style={{ background: r.test ? '#BFDBFE' : '#F1F4F9' }}
            />
            {r.text}
          </span>
        ))}
    ...
    <div className="ns-login-shell" data-testid="login-screen">
      ...
      <div className="ns-login-card w-full max-w-md mx-auto z-10 relative">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div className="leading-none">
            <p className="font-bold text-xl tracking-tight text-[#007AFF]">
              Noni<span className="text-[#4A5462]">.</span>
            </p>
            <p className="text-[9px] font-bold uppercase tracking-[0.08em] text-[#94A1B2] mt-1">
              Sistema de reservas
            </p>
          </div>
        </div>
        ...
        {/* Title */}
        <div className="mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-[#1D212A] leading-[1.05] mb-2">
            {configUI[mode].titulo}{' '}
            <span className="text-[#4A5462]">
              {configUI[mode].acento}
              {mode === 'login' || mode === 'registro' ? '.' : ''}
            </span>
          </h2>
          <p className="text-[13px] font-medium text-[#68748A] leading-relaxed">
        ...
        {mensaje && (
          <div
            ...
            style={{
              background:
                mensaje.tipo === 'error' ? 'rgba(0,122,255,0.06)' : 'rgba(0,122,255,0.06)',
              color: mensaje.tipo === 'error' ? '#93C5FD' : '#BFDBFE',
              borderColor:
                mensaje.tipo === 'error' ? 'rgba(0,122,255,0.15)' : 'rgba(0,122,255,0.15)',
            }}
          >
            ...
            <div className="flex-1">
              ...
              {cooldown > 0 && mensaje.tipo === 'error' && (
                <div className="mt-2 h-1 w-full rounded-full overflow-hidden bg-[#F1F4F9]">
                  <div
        ...
        {mode !== 'recuperar' && (
          <>
            ...
            <div className="relative mb-6 flex items-center gap-3">
              <div className="flex-1 h-px bg-[#F1F4F9]" />
              <span className="text-[10px] font-bold uppercase tracking-[0.09em] text-[#94A1B2]">O con email</span>
              <div className="flex-1 h-px bg-[#F1F4F9]" />
            </div>
          </>
        )}
        ...
        <form onSubmit={handleAuth} className="space-y-4" data-testid="auth-form">
          <div>
            <label htmlFor="email" className="block text-[10px] font-bold uppercase tracking-[0.08em] text-[#94A1B2] mb-2">Email</label>
            <input
              id="email"
              type="email"
              required
          ...
          {mode === 'registro' && (
            <div>
              <label htmlFor="confirmPassword" className="block text-[10px] font-bold uppercase tracking-[0.08em] text-[#94A1B2] mb-2">Confirmar contraseña</label>
              <input
                id="confirmPassword"
                type={showPassword ? 'text' : 'password'}
                required
                ...
                className="ns-login-input"
                style={confirmPassword.length > 0 && password !== confirmPassword ? { borderColor: '#93C5FD' } : undefined}
              />
            </div>
          )}
          ...
          {mode === 'registro' && (
            <label className="flex items-start gap-3 cursor-pointer select-none pt-1">
              <input
                ...
                className="mt-0.5 w-4 h-4 rounded cursor-pointer accent-[#4A9CFF]"
              />
              <span className="text-[12px] leading-snug text-[#68748A]">
                Acepto los{' '}
                <button type="button" onClick={() => setLegalSheet('terms')} className="font-semibold underline-offset-4 hover:underline text-[#4A5462]">Términos</button>
                {' '}y la{' '}
                <button type="button" onClick={() => setLegalSheet('privacy')} className="font-semibold underline-offset-4 hover:underline text-[#4A5462]">Política de Privacidad</button>.
              </span>
            </label>
          )}
        ...
        {/* Switch */}
        <div className="mt-8 pt-6 border-t border-[#F1F4F9]">
          {mode === 'recuperar' ? (
            <button
              type="button"
              onClick={() => cambiarModo('login')}
              className="text-[11px] font-bold uppercase tracking-[0.06em] text-[#94A1B2] hover:text-[#007AFF] transition-all flex items-center gap-2"
            >
            ...
            <p className="text-[11px] font-bold uppercase tracking-[0.06em] text-[#94A1B2]">
        ...
        {/* Footer */}
        <div className="mt-8 pt-4 border-t border-[#F1F4F9] flex items-center justify-between text-[9px] uppercase tracking-[0.08em] text-[#94A1B2]">
      ...
      {legalSheet && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-5" role="presentation">
          <button type="button" aria-label="Cerrar información legal" className="absolute inset-0 bg-[#007AFF]/45 backdrop-blur-sm" onClick={() => setLegalSheet(null)} />
          <section role="dialog" aria-modal="true" aria-labelledby="legal-sheet-title" className="relative w-full sm:max-w-md rounded-t-[2rem] sm:rounded-[2rem] bg-white p-6 sm:p-7 shadow-2xl animate-in slide-in-from-bottom-6 duration-300">
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="inline-flex rounded-full bg-[#F8ECEC] px-3 py-1 text-[9px] font-bold uppercase tracking-[0.06em] text-[#4A5462]">Noni</span>
                <h2 id="legal-sheet-title" className="mt-3 text-xl font-bold tracking-tight text-[#1D212A]">
                  {legalSheet === 'terms' ? 'Términos de uso' : 'Política de privacidad'}
                </h2>
              </div>
              <button type="button" aria-label="Cerrar" onClick={() => setLegalSheet(null)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#F8FAFC] text-[#4A5462] transition-colors hover:bg-[#E4EAF2]">
            ...
            <div className="mt-5 space-y-3 text-sm leading-relaxed text-[#4A5462]">
              {legalSheet === 'terms' ? (
                <>
            ...
            <button type="button" onClick={() => setLegalSheet(null)} className="mt-6 min-h-11 w-full rounded-2xl bg-[#4A9CFF] px-4 py-3 text-xs font-bold uppercase tracking-[0.13em] text-white shadow-[0_10px_22px_rgba(16,24,40,0.25)] transition-transform active:scale-[0.98]">Entendido</button>
          </section>
        </div>
      )}
    </div>
