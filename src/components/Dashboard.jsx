...
export default function Dashboard({ session }) {
  ...
  return (
    <div className={`min-h-screen font-sans antialiased ${negocio?.es_admin_plataforma ? 'bg-[#0A0A0B] text-slate-100' : negocio ? 'ns-owner-dark' : 'bg-[#F8FAFC] text-slate-900'}`} style={{ colorScheme: (negocio?.es_admin_plataforma || negocio) ? 'dark' : 'light' }}>
      ...
      <nav className={`h-14 md:h-16 border-b flex items-center justify-between px-4 md:px-6 sticky top-0 z-50 ${negocio?.es_admin_plataforma ? 'bg-[#0A0A0B]/90 backdrop-blur-md border-white/5 shadow-2xl' : negocio ? 'ns-nav-dark' : 'bg-white/90 backdrop-blur-md border-slate-200 shadow-sm'}`}>
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className={`h-3 md:h-4 w-px mx-1 ${negocio ? 'bg-white/10' : 'bg-slate-200'}`}></div>
        ...
        <div className="flex items-center gap-3">
          {negocio && !negocio.es_admin_plataforma && (
            <>
              ...
              <button onClick={() => setSearchOpen(true)} className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 transition-all text-[10px] font-medium text-white/50 border border-white/10">
      ...
      <main className={`max-w-7xl mx-auto p-4 md:p-8 ${!negocio?.es_admin_plataforma && negocio ? 'ns-has-bottom-nav' : ''}`}>
        ...
        {!negocio ? (
          ...
          <div className="space-y-4 md:space-y-6 animate-in slide-in-from-bottom-8 duration-700">
            ...
            {tab !== 'inicio' && (
            <header className="ns-hero-compact relative overflow-hidden rounded-[1.3rem] md:rounded-[2rem] group animate-in fade-in duration-500 nh-hero">
              ...
              <div className="relative z-10 flex items-center justify-between gap-3 md:gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 md:mb-3">
                    ...
                    <span className="nh-eyebrow">{tabsConfig.find(t => t.id === tab)?.label || 'Panel'}</span>
                  </div>
                  <h2 className="nh-title truncate">{negocio.nombre}</h2>
                  <p className="nh-date">{negocio.rubro}</p>
                </div>
                {logoUrl && (
                  <div className="nh-logo shrink-0">
                    <img src={logoUrl} className="w-full h-full object-cover" alt="Logo" />
                  </div>
                )}
              </div>
              <div className="absolute -top-20 -right-20 md:-top-28 md:-right-28 w-64 h-64 md:w-80 md:h-80 rounded-full blur-[80px] md:blur-[110px] opacity-30 group-hover:scale-110 transition-transform duration-1000 pointer-events-none" style={{ background: colorPrimario }}></div>
            </header>
            )}
      ...
      {negocio && !negocio.es_admin_plataforma && (
        <nav className="ns-bottom-nav ns-bottom-nav--dark md:hidden">
          {bottomNavTabs.map(item => (
            ...
            >
              ...
              {tab === item.id && <div className="w-1 h-1 rounded-full bg-white mt-px"></div>}
            </button>
          ))}
        </nav>
      )}
