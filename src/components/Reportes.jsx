    ...
    <div className="flex flex-col gap-4 ns-tab-content-enter pb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="ns-kpi-card ui-tile ui-tile--green ns-stagger-in ns-delay-1">
          <div className="flex items-start justify-between mb-3 relative z-10">
            ...
            <span
              className={`ui-chip ${variacionIngresos >= 0 ? 'ui-chip--solid' : 'ui-chip--outline'}`}
        ...
        <div className="ns-kpi-card ui-tile ui-tile--blue ns-stagger-in ns-delay-2">
          <div className="flex items-start justify-between mb-3 relative z-10">
            ...
            <span
              className={`ui-chip ${variacionTurnos >= 0 ? 'ui-chip--solid' : 'ui-chip--outline'}`}
      ...
      <div className="ns-kpi-card ns-stagger-in ns-delay-4">
        <div className="flex items-center justify-between mb-6 relative z-10">
          <div>
            <h4 className="text-base md:text-lg font-bold tracking-tight" style={{ color: 'var(--ns-text)' }}>
                ...
                <div className="w-full relative h-full flex items-end">
                  ...
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: d.valor > 0 ? '0 -2px 8px rgba(16,24,40,0.2)' : 'none',
                    }}
                  />
                </div>
                <span className="text-[7px] font-bold uppercase tracking-tight" style={{ color: isToday ? 'var(--ns-primary)' : 'var(--ns-text-muted)' }}>
                  {d.label}
                </span>
              </div>
            )
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="ns-kpi-card ns-stagger-in ns-delay-5 p-0 overflow-hidden">
          <div className="p-5 md:p-6 border-b border-[var(--ns-line)] relative z-10" style={{ borderColor: 'var(--ns-border)' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold tracking-tight" style={{ color: 'var(--ns-text)' }}>{vocab.servicioPlural} más rentables</h4>
                <p className="text-[9px] font-bold uppercase tracking-[0.06em]" style={{ color: 'var(--ns-text-muted)' }}>Por facturación</p>
              </div>
            </div>
          </div>
          {datos.topServicios.length === 0 ? (
            <div className="ui-empty">
              <p className="ui-empty__title">Todavía sin datos</p>
              <p className="ui-empty__text">Cuando se confirmen turnos en este período, el ranking aparece acá.</p>
            </div>
          ) : (
        ...
        <div className="ns-kpi-card ns-stagger-in ns-delay-6 p-0 overflow-hidden">
          <div className="p-5 md:p-6 border-b border-[var(--ns-line)] relative z-10" style={{ borderColor: 'var(--ns-border)' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold tracking-tight" style={{ color: 'var(--ns-text)' }}>{vocab.empleadoPlural} por rendimiento</h4>
                <p className="text-[9px] font-bold uppercase tracking-[0.06em]" style={{ color: 'var(--ns-text-muted)' }}>Por facturación generada</p>
              </div>
            </div>
          </div>
                ...
                return (
                  <div key={idx} className="px-5 py-4 flex items-center gap-3 transition-colors hover:bg-[var(--ns-accent-bg)]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="ui-progress mt-2" style={{ height: 7 }}>
                        <div className="ui-progress__fill" style={{ width: `${(e.revenue / maxRev) * 100}%` }} />
                      </div>
                    </div>
                    <div className="text-right shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="ns-kpi-card ns-stagger-in ns-delay-7">
          <div className="flex items-center gap-3 mb-5 relative z-10">
            ...
            <div>
              <h4 className="text-sm font-bold tracking-tight" style={{ color: 'var(--ns-text)' }}>Horas con más demanda</h4>
              <p className="text-[9px] font-bold uppercase tracking-[0.06em]" style={{ color: 'var(--ns-text-muted)' }}>Distribución horaria</p>
            </div>
          </div>
          ...
          {Object.keys(datos.horasPico).length === 0 ? (
            <div className="ui-empty">
              <p className="ui-empty__title">Todavía sin datos</p>
              <p className="ui-empty__text">Con los primeros turnos vas a ver en qué horas se te llena la agenda.</p>
            </div>
          ) : (
              ...
              {Object.entries(datos.horasPico)
                ...
                .map(([hora, count], idx) => (
                  <div key={hora} className="flex items-center gap-3">
                    <span className="text-[11px] font-bold w-12 shrink-0 tabular-nums" style={{ color: idx === 0 ? 'var(--ns-primary)' : 'var(--ns-text-muted)' }}>{hora}:00</span>
                    <div className="ui-progress flex-1">
                      <div className="ui-progress__fill" style={{ width: `${(count / maxHoraPico) * 100}%`, opacity: idx === 0 ? 1 : 0.65 }} />
                    </div>
                    <span className="text-[11px] font-bold w-5 text-right" style={{ color: 'var(--ns-text)' }}>{count}</span>
                  </div>
                ))}
            </div>
          )}
        ...
        <div className="ns-kpi-card ns-stagger-in ns-delay-8 p-0 overflow-hidden">
          <div className="p-5 md:p-6 border-b border-[var(--ns-line)] relative z-10" style={{ borderColor: 'var(--ns-border)' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold tracking-tight" style={{ color: 'var(--ns-text)' }}>{mayusculaInicial(vocab.clientePlural.replace('Base de ', ''))} destacados</h4>
                <p className="text-[9px] font-bold uppercase tracking-[0.06em]" style={{ color: 'var(--ns-text-muted)' }}>Mayor facturación</p>
              </div>
            </div>
          </div>
            ...
            <div className="divide-y relative z-10" style={{ borderColor: 'var(--ns-border)' }}>
              {datos.topClientes.map((c, idx) => (
                <div key={idx} className="px-5 py-4 flex items-center gap-3 transition-colors hover:bg-[var(--ns-accent-bg)]">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-bold truncate leading-tight" style={{ color: 'var(--ns-text)' }}>{c.nombre}</p>
